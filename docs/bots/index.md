# Bots y Automatizaciones

Documentación interna de bots, asistentes y automatizaciones usados por el equipo de Sandra Vargas.

## Bots disponibles

- [AI Chatbot + KB Builder Prompts](./ai-chatbot-kb-builder-prompts.md): prompt base para chatbot y plantilla de knowledge base builder.
- [AI Chatbot + KB Builder Prompts (ES)](./ai-chatbot-kb-builder-prompts-es.md): version en espanol del prompt base para chatbot y plantilla de knowledge base builder.
- [Laura](./laura/index.md): configuración base del bot de atención y precalificación para leads inmobiliarios.
- [Laura Internacional](./laura-internacional/index.md): flujo de atencion, precalificacion y agendamiento para leads internacionales interesados en invertir en Florida.
- [Exportar conversaciones desde GHL](./exportar-conversaciones-ghl.md): script para bajar conversaciones recientes y generar un `.txt` por bot.
